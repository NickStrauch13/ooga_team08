...
public class GhostSlowerTest extends DukeApplicationTest {
    ...
    @Test
    public void ghostSlowerInteractTest(){
        newBoard.getGameObject(0,3).interact(myController.getGameController().getGame());
        int CPUspeed=0;
        for (CPUCreature cpu : myController.getGameController().getGame().getCPUs()){
            CPUspeed = (int)cpu.getSpeed();
            break;
        }
        assert (CPUspeed == 2);
    }
